...
export function evaluateBrandOrderAuthorization(order, adminBrand) {
  ...
  for (const item of order.items) {
    if (item.collaborationProductId || item.collaborationProduct) {
      ...
      if (adminBrandKey === brandA || adminBrandKey === brandB) {
        ...
        hasValidCollaborationForAdmin = true;
      } else {
        hasOtherBrandItem = true;
      }
      ...
      // Unrecognized or unpopulated item brand strictly fails closed as other brand item
      hasOtherBrandItem = true;
    }
  }
  ...
  if (hasOtherBrandItem && !hasValidCollaborationForAdmin) {
    return { authorized: false, reason: "mixed_brand_forbidden" };
  }
