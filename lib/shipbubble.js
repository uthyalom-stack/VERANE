 ...
 * into a Shipbubble address_code dynamically using Shipbubble Address API.
 *
 * Official Shipbubble API Endpoint:
 * POST https://api.shipbubble.com/v1/shipping/address
 *
...
export async function resolveShipbubbleAddressCode(addressObj) {
  ...
  if (!addressObj || !addressObj.state || !addressObj.city || !addressObj.address) {
    throw new Error("Valid physical address object (name, phone, email, address, city, state) is required for address code resolution.");
  }

  const payload = {
    name: addressObj.name || "Customer",
    email: addressObj.email || "customer@example.com",
  ...
  const response = await fetch(`${SHIPBUBBLE_API_BASE}/shipping/address`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok || data.status === "error" || data.status === false) {
    throw new Error(data.message || "Failed to resolve Shipbubble address code.");
  }
  ...
  if (!addressCode) {
    throw new Error("Shipbubble Address API did not return a valid address_code.");
  }
