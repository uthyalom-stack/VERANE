 ...
 * Throws clear error if physical origin is unconfigured.
 */
export async function getShipbubbleOriginAddress() {
  if (isMockMode()) {
    return {
      name: "VÉRANE Fulfillment Center",
      email: "orders@verane.com",
      phone: "+2348000000000",
      country: "Nigeria",
      state: "Lagos",
      city: "Victoria Island",
      address: "Atelier Street, Victoria Island",
    };
  }
  ...
  if (!state || !city || !street) {
    throw new Error("Shipbubble physical delivery origin address (State, City, Street) is not configured in site settings.");
  }

  return {
    name: name || "VÉRANE House",
    email: email || "orders@verane.com",
 ...
 *
 * @param {Object} params
 * @param {Object|string} [params.senderAddress] - Physical origin object or dynamic code
...
export async function fetchShipbubbleRates({ senderAddress, receiverAddress, packageItems, packageDimension }) {
  ...
  if (typeof origin === "string") {
    payload.sender_address_code = origin;
  } else {
    payload.sender_address = origin;
  }
