...
export async function fetchShippingRates(params) {
  ...
  if (senderAddressCode) {
    payload.sender_address_code = senderAddressCode;
  } else if (shipFrom) {
    payload.ship_from = shipFrom;
  } else {
    throw new Error("Either senderAddressCode or shipFrom is required for fetching rates.");
  }

  if (recieverAddressCode) {
    payload.reciever_address_code = recieverAddressCode;
  } else if (shipTo) {
    payload.ship_to = shipTo;
  } else {
    throw new Error("Either recieverAddressCode or shipTo is required for fetching rates.");
  }

  payload.package_items = packageItems || [];
  payload.package_dimension = packageDimension || { length: 20, width: 20, height: 10 };
