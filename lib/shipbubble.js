...
/**
 * Fetches available package categories from Shipbubble Categories API.
 * Official Endpoint: GET https://api.shipbubble.com/v1/shipping/categories
 *
 * @returns {Promise<Array<{category_id: string|number, name: string}>>}
 */
export async function getShipbubbleCategories() {
  if (isMockMode()) {
    return [
      { category_id: "cat_apparel_fashion", name: "Apparel & Fashion Accessories" },
      { category_id: "cat_footwear_shoes", name: "Footwear & Shoes" },
      { category_id: "cat_general_merchandise", name: "General Merchandise" },
    ];
  }
  ...
  const response = await fetch(`${SHIPBUBBLE_API_BASE}/shipping/categories`, {
    method: "GET",
    headers: {
  ...
  return categories.map((cat) => ({
    category_id: cat.category_id || cat.id || cat.code,
 ...
 *
 * In production mode, if shipbubbleCategoryId is not configured in environment or site settings,
 * an explicit configuration error is thrown rather than assuming a hardcoded fallback.
 */
export async function getShipbubbleCategoryId() {
  if (process.env.SHIPBUBBLE_CATEGORY_ID) {
    return process.env.SHIPBUBBLE_CATEGORY_ID.trim();
  }

  try {
    const setting = await prisma.siteSetting.findUnique({
      where: { key: "shipbubbleCategoryId" },
    });
  ...
  if (isMockMode()) {
    return "mock_cat_fashion";
  }

  throw new Error(
    "Shipbubble Category ID is not configured. Please configure shipbubbleCategoryId in Admin Settings or set the SHIPBUBBLE_CATEGORY_ID environment variable."
  );
}
...
export async function fetchShipbubbleRates({ senderAddress, receiverAddress, packageItems, packageDimension }) {
  ...
  const categoryId = await getShipbubbleCategoryId();
