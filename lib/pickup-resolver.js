 ...
 *    (Uses explicit Collaboration.creatorRole or Collaboration.creatorBrand, without falling back to brandA or requests)
...
export async function resolveOrderPickupBrand(items) {
  ...
  for (const item of items) {
    ...
    if (collabId && item.isCollaboration) {
      ...
      // Resolve creator identity strictly from creatorRole or creatorBrand
      const rawCreator = collab.creatorRole || collab.creatorBrand;
      if (!rawCreator) {
        throw new Error(`Collaboration "${collab.name}" (ID: ${collab.id}) does not have an explicit creator registered.`);
      }

      const normalized = String(rawCreator).trim().toUpperCase();
      if (normalized.includes("ALOMZIEE")) {
        collaborationCreatorBrand = "ALOMZIEE";
      } else if (normalized.includes("UTHY")) {
        collaborationCreatorBrand = "UTHY";
      } else {
        throw new Error(`Invalid collaboration creator identity "${rawCreator}" for collaboration "${collab.id}".`);
      }
    } else {
