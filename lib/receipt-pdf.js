...
/**
 * Extracts natural dimensions and aspect ratio from a PNG or JPEG image buffer.
 * @param {Buffer} buffer - The image data to inspect.
 * @param {string} format - The image format, such as `PNG` or `JPEG`.
 * @returns {{width: number, height: number, aspectRatio: number}|null} The image dimensions and aspect ratio, or `null` if they cannot be determined.
 */
...
/**
 * Prepares a supported image for embedding in a PDF.
 * @param {string} url - A remote image URL or base64 data URL.
 * @return {{data: string, format: string, aspectRatio: number}|null} The image data URI, format, and aspect ratio, or `null` if the image is invalid or unavailable.
 */
...
/**
 * Generate a two-page PDF receipt for an order.
 * @param {string} orderId - The identifier of the order to include.
 * @return {Promise<Buffer>} The generated PDF as a Node.js buffer.
