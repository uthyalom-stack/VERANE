...
export async function generateOrderReceiptPDF(orderId) {
  // 1. Fetch order details with items, product, user, and fulfillment details
  ...
  // 6. FULFILLMENT & LOGISTICS SPECIFICATION
  rowY += 16;
  doc.setFont("helvetica", "bold");
  doc.setFontSize(7.5);
  doc.setTextColor(160, 140, 70);
  ...
  const isPickup = ful?.fulfillmentType === "pickup";

  doc.text(isPickup ? "CUSTOMER PICKUP LOCATION & SCHEDULE" : "DELIVERY DESTINATION", col1, rowY);
  ...
  if (isPickup) {
    destText = [
      `Fulfillment: Pickup (Free: NGN 0)`,
      `Location: ${ful.pickupBrand || "VÉRANE"} Atelier`,
      `Pickup Address: ${ful.pickupAddress || order.address || "Atelier Location"}`,
      `Pickup Date: ${ful.requestedPickupDate || "Scheduled Date"}`,
    ].join("\n");
  } else {
    destText = [
      `Fulfillment: Delivery via Courier`,
      `Recipient: ${customerName}`,
      `Address: ${order.address || "Physical Address"}`,
      `Destination: ${order.city ? `${order.city}${order.state ? `, ${order.state}` : ""}` : order.state || "Lagos"}, ${order.country || "Nigeria"}`,
    ].join("\n");
  }

  const splitDest = doc.splitTextToSize(destText, pageWidth - 56);
