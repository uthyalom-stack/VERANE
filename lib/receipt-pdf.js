...
export async function generateOrderReceiptPDF(orderId) {
  ...
  const settings = {};
  settingRows.forEach((r) => { settings[rowKey(r.key)] = r.value; });

  function rowKey(k) { return k; }
  ...
  doc.roundedRect(25, 108, pageWidth - 50, 16, 3, 3, "F");
  doc.setFont("times", "bold");
  doc.setFontSize(10);
  doc.setTextColor(180, 140, 30);
  doc.text(badgeText, pageWidth / 2, 118, { align: "center" });
