 ...
 */
export async function calculateOrderTotalsServer({
  items,
  fulfillmentMethod,
  selectedCourierId,
  selectedCourierCode,
  ...
  // Determine Authoritative Shipping Fee Server-Side
  ...
  if (method === "PICKUP") {
    ...
    // DELIVERY METHOD: Re-query live Shipbubble rate or fallback to local table lookup
    if (selectedCourierId) {
      try {
        const { fetchShippingRates } = await import("@/lib/shipbubble");
        const { getShipbubbleSenderAddress } = await import("@/lib/fulfillment");

        const shipFrom = await getShipbubbleSenderAddress();
        const fullRecipientAddress = `${address || ""}, ${city || ""}, ${state || ""}, ${country || "Nigeria"}`;

        const ratesRes = await fetchShippingRates({
          shipFrom,
          shipTo: {
          ...
          packageItems: validatedItems.map((i) => ({
            name: String(i.name || "VÉRANE Garment").slice(0, 50),
            quantity: Number(i.qty || 1),
            amount: Number(i.price || 0) * Number(i.qty || 1),
          })),
        });

        const couriers = Array.isArray(ratesRes.data?.couriers) ? ratesRes.data.couriers : [];
        const matchedCourier = couriers.find((c) => String(c.courier_id) === String(selectedCourierId));

        if (matchedCourier) {
          shippingFee = Number(matchedCourier.rate_card_amount || matchedCourier.total || 0);
          quotedCost = Number(matchedCourier.total || matchedCourier.rate_card_amount || 0);
        } else {
          // Fall back to static state calculation if courier not found in re-fetch
          shippingFee = 5000;
          quotedCost = 5000;
        }
      } catch (rateErr) {
        console.error("Live rate re-validation fallback:", rateErr);
        shippingFee = 5000;
        quotedCost = 5000;
      }
    } else {
      // Static fallback if no courier ID selected
      ...
      if (reqCountry.toLowerCase() === "nigeria") {
        ...
        if (matchedStateKey) {
          const dbState = await prisma.deliveryState.findUnique({
            ...
            include: { cities: { where: { enabled: true } } },
          });
          ...
          if (dbState && dbState.enabled) {
            if (dbState.pricingMode === "CITY_SPECIFIC" && reqCity) {
              ...
              shippingFee = matchedCity && Number(matchedCity.fee) >= 0 ? Number(matchedCity.fee) : Number(dbState.defaultFee || 0);
            } else {
