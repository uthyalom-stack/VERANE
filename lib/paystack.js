...
export async function calculateOrderTotalsServer({ items, fulfillmentType = "delivery", selectedCourier, requestedPickupDate, receiverAddress }) {
  ...
  for (const item of items) {
    ...
    } else if (isCollaboration && (item.productA || item.productB)) {
      let resolvedCollabWeight = null;
      if (item.productA) {
        try {
          resolvedCollabWeight = resolveItemUnitWeight(item.productA);
        } catch {
          // Keep checking productB if productA weight is unconfigured
        }
      }
      if (!resolvedCollabWeight && item.productB) {
        resolvedCollabWeight = resolveItemUnitWeight(item.productB);
      }

      collabProductRef = {
        name: item.name || "Collaboration Item",
        price: unitPrice,
        weight: resolvedCollabWeight,
        packageLength: item.productA?.packageLength || 20,
        packageWidth: item.productA?.packageWidth || 20,
        packageHeight: item.productA?.packageHeight || 5,
      };
    }
  ...
  if (normalizedMode === "pickup") {
    ...
    const recName = `${receiverAddress?.firstName || ""} ${receiverAddress?.lastName || ""}`.trim();
    ...
    const packageItems = validatedItems.map((it) => {
      ...
      return {
        ...
        unit_price: Math.round(Number(it.price)), // it.price was computed server-side from product.price or collabProduct.price
    ...
    if (!matched) {
      throw new Error("Selected courier shipping option is no longer valid. Please recalculate rates.");
    }
