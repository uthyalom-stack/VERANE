...
/**
 * Validates cart items and calculates subtotal and order totals.
 ...
 *
 * @param {Object} options - Cart information.
 * @param {Array} options.items - Items included in the cart.
 * @returns {Promise<Object>} The subtotal, shipping fee (0), total, and validated cart items.
 */
...
export async function calculateOrderTotalsServer({ items, fulfillmentType = "delivery", selectedCourier, requestedPickupDate, receiverAddress }) {
  ...
  if (normalizedMode === "pickup") {
    ...
    // Server verifies quote by fetching fresh rates if receiverAddress is present
    let verifiedCourier = null;
    if (receiverAddress && receiverAddress.state && receiverAddress.city) {
      ...
      const freshRates = await fetchShipbubbleRates({
        senderAddress: sender,
        receiverAddress: receiver,
        parcels: [
          {
            weight: Math.max(1, validatedItems.reduce((sum, item) => sum + Number(item.qty || item.quantity || 1), 0)),
            length: 15,
            width: 15,
            height: 15,
          },
      ...
      const matched = freshRates.couriers.find(
        (c) => c.courier_id === selectedCourier.courier_id && c.service_code === selectedCourier.service_code
      );

      if (matched) {
        verifiedCourier = {
          ...matched,
          request_token: freshRates.request_token,
        };
      } else if (freshRates.couriers && freshRates.couriers.length > 0) {
        throw new Error("Selected courier shipping option is no longer available. Please recalculate rates.");
      }
    }

    if (!verifiedCourier) {
      if (!selectedCourier.total_charge || isNaN(Number(selectedCourier.total_charge))) {
        throw new Error("Invalid courier shipping charge.");
      }
      verifiedCourier = {
        courier_id: String(selectedCourier.courier_id),
        courier_name: String(selectedCourier.courier_name || "Courier"),
        service_code: String(selectedCourier.service_code),
        service_type: String(selectedCourier.service_type || "Delivery"),
        total_charge: Math.max(0, Number(selectedCourier.total_charge)),
        request_token: String(selectedCourier.request_token || ""),
      };
    }

    shippingFee = Math.max(0, Number(verifiedCourier.total_charge || 0));

    fulfillmentDetails = {
      fulfillmentType: "delivery",
      courierId: verifiedCourier.courier_id,
      courierName: verifiedCourier.courier_name,
      serviceCode: verifiedCourier.service_code,
      serviceName: verifiedCourier.service_type,
      rateToken: verifiedCourier.request_token,
    };
  }
