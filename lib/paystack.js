...
export async function calculateOrderTotalsServer({ items, country, state, city, zone }) {
  ...
  for (const item of items) {
    ...
    if (isCollaboration && collaborationProductId) {
      ...
      const isPreOrder = Boolean(item.isPreOrder || product.preOrderEnabled);
      ...
      if (variantId) {
        ...
        // Product has variants: source of truth for stock is sum of variant stocks
    ...
    validatedItems.push({
      ...
      qty,
      price: unitPrice,
      isPreOrder: Boolean(item.isPreOrder || (productId && !isCollaboration ? (await prisma.product.findUnique({ where: { id: productId }, select: { preOrderEnabled: true } }))?.preOrderEnabled : false)),
  ...
  const finalizedOrder = await prisma.$transaction(async (tx) => {
    ...
    for (const item of items) {
      ...
      if (item.collaborationVariantId) {
        ...
        if (product && !isPreOrder && product.inventory < qty) {
