...
/**
 * Selects the configured Reply-To address for outgoing email, if available.
 * @return {string|undefined} The configured reply-to address, or `undefined`.
 */
function getReplyToAddress() {
  const replyTo = process.env.RESEND_REPLY_TO || process.env.SMTP_REPLY_TO || process.env.SUPPORT_EMAIL || "";
...
export async function sendWelcomeEmail({ email, name }) {
  ...
  const text = `WELCOME TO VÉRANE\n\nDear ${customerName},\n\nThank you for creating an account with VÉRANE. You now have access to exclusive capsule releases from UTHY LUXURY and ALOMZIEE FOOTIES.\n\nExplore Catalog: ${baseUrl}/catalog`;
...
export async function sendOrderReceiptEmail({ order, pdfBuffer }) {
  ...
  const formattedTotal = Number(order.total || 0).toLocaleString('en-NG');
  const text = `VÉRANE — ORDER CONFIRMATION\n\nORDER #${order.orderNumber} CONFIRMED\n\nDear ${customerName},\n\nYour payment was successful. We have attached your official VÉRANE PDF Receipt and item specifications to this email.\n\nTOTAL PAID: ₦${formattedTotal}\n\nView Order Status: ${baseUrl}/orders?order=${order.orderNumber}`;
      ...
      <body style="background-color: #000; color: #fff; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; padding: 40px; text-align: center;">
        <div style="max-width: 600px; margin: 0 auto; border: 1px solid #222; border-radius: 20px; padding: 40px; background-color: #0a0a0a;">
          ...
          <div style="margin-top: 30px; padding: 20px; border-radius: 12px; background-color: #111; text-align: left;">
            ...
            <p style="font-size: 20px; font-weight: bold; color: #f5b942; margin: 5px 0 0 0;">₦${formattedTotal}</p>
          </div>
