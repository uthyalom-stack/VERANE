...
export async function sendWelcomeEmail({ email, name }) {
  ...
  try {
    ...
    if (response.error) {
      console.error("Resend API welcome email error:", response.error);
    ...
    console.error("Resend SDK welcome email send exception:", error);
...
export async function sendOrderReceiptEmail({ order, pdfBuffer }) {
  ...
  const attachments = [];
  if (pdfBuffer) {
    // Resend SDK accepts buffer objects in content
    attachments.push({
  ...
  try {
    ...
    if (response.error) {
      console.error("Resend API order receipt email error:", response.error);
      return { success: false, error: response.error };
    }
    ...
    console.error("Resend SDK order receipt email send exception:", error);
    return { success: false, error: error?.message || error };
  }
}
