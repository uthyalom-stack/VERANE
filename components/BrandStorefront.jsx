      ...
      <main className="min-h-screen bg-black text-white">
        <div className="mx-auto max-w-3xl px-6 py-24 text-center">
          ...
          <Link
            ...
            className="mt-6 inline-block rounded-full bg-white px-6 py-3 text-xs font-bold text-black"
          >
            Shop Products
          </Link>
        </div>
        ...
        {sections.length === 0 ? (
          <section className="py-20 text-center">
            <p className="text-sm text-white/30">
              This store has no featured sections yet.
            </p>
            <Link
              href={`/catalog?brand=${encodeURIComponent(brand)}`}
              className="mt-6 inline-block rounded-full bg-white px-6 py-3 text-xs font-bold uppercase tracking-[0.12em] text-black"
            >
              Browse Products
            </Link>
          </section>
          ...
          sections.map((section) => (
            <section key={section.id} className="mb-20 last:mb-0">
              ...
              <div className="mb-7 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                ...
                <Link
                  href={`/catalog?brand=${encodeURIComponent(brand)}`}
                  className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/40 transition hover:text-white"
                >
                  View all
                </Link>
              </div>
