  ...
  useEffect(() => {
    ...
    async function loadStorefront() {
      try {
        ...
        const response = await fetch(`/api/storefront/${brand}`, { cache: "no-store" });
    ...
    return (
      <main className="min-h-screen bg-black px-6 py-24 text-white">
        <div className="mx-auto max-w-xl rounded-3xl border border-white/10 bg-white/[0.03] p-10 text-center">
          ...
          <Link href="/catalog" className="mt-6 inline-flex rounded-full bg-white px-6 py-3 text-xs font-bold uppercase tracking-[0.15em] text-black">
            Browse Catalog
          </Link>
        </div>
      </main>
    ...
    <main className="min-h-screen bg-black text-white">
      ...
      <div className="mx-auto max-w-7xl px-5 py-14 sm:px-8 lg:px-10">
            ...
            {sections.map((section) => (
              <section key={section.id}>
                <div className="mb-7 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                  <div>
                    <h2 className="text-2xl font-semibold tracking-[-0.03em] sm:text-3xl">{section.title || "Featured"}</h2>
                    {section.description && <p className="mt-2 text-sm text-white/35">{section.description}</p>}
                  </div>
                  <Link href={`/catalog?brand=${encodeURIComponent(brand)}`} className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/40 hover:text-white">View all</Link>
                </div>
        ...
        <div className="mt-20 border-t border-white/10 pt-8">
          <Link href={`/catalog?brand=${encodeURIComponent(brand)}`} className="inline-flex rounded-full border border-white/15 px-6 py-3 text-[10px] font-bold uppercase tracking-[0.15em] text-white transition hover:bg-white hover:text-black">
            View Full Catalog
          </Link>
        </div>
      </div>
