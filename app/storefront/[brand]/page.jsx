  ...
  useEffect(() => {
    ...
    async function loadStorefront() {
      try {
        ...
        const response = await fetch(`/api/storefront/${brand}`, {
          cache: "no-store",
        });
        ...
        if (!response.ok) {
          throw new Error(result?.error || "Failed to load store.");
        }
    ...
    return (
      <main className="min-h-screen bg-black text-white px-6 py-24">
        <div className="mx-auto max-w-xl rounded-3xl border border-white/10 bg-white/[0.03] p-10 text-center">
          ...
          <Link
            href="/catalog"
            className="mt-6 inline-flex rounded-full bg-white px-6 py-3 text-xs font-bold uppercase tracking-[0.15em] text-black"
          >
            Browse Catalog
          </Link>
        </div>
    ...
    <main className="min-h-screen bg-black text-white">
      <section className="border-b border-white/10">
        <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-10 lg:py-28">
          <div className="max-w-3xl">
            <p className="mb-5 text-[9px] font-bold uppercase tracking-[0.35em] text-amber-400">
              {brand}
            </p>
      ...
      <div className="mx-auto max-w-7xl px-5 py-14 sm:px-8 lg:px-10">
        {sections.length > 0 ? (
              ...
              <section key={section.id}>
                <div className="mb-7 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                  ...
                  <Link
                    ...
                    className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/40 hover:text-white"
                  >
                    View all
                  </Link>
                </div>
          ...
          <section>
            <div className="mb-7 flex items-end justify-between gap-4">
              <div>
                <h2 className="text-2xl font-semibold">Shop {brandInfo.name}</h2>
                <p className="mt-2 text-sm text-white/35">
                  Explore everything currently available from this store.
                </p>
              </div>
            </div>
              ...
              {products.map((product) => {
                ...
                return (
                  <Link key={product.id} href={`/catalog/${product.id}`} className="group block">
                    ...
                    <div className="pt-3">
                      <p className="truncate text-sm font-medium">{product.name || "Unnamed Product"}</p>
        ...
        <div className="mt-20 border-t border-white/10 pt-8">
          <Link
            href={`/catalog?brand=${encodeURIComponent(brand)}`}
            className="inline-flex rounded-full border border-white/15 px-6 py-3 text-[10px] font-bold uppercase tracking-[0.15em] text-white transition hover:bg-white hover:text-black"
          >
            View Full Catalog
          </Link>
        </div>
