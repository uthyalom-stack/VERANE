...
/**
 * Render the product catalog with client interactivity (filtering, sorting, wishlist, quick add).
 * Initial products and collections are provided directly from Server Component.
 */
export default function CatalogClient({
  ...
  /*
   * Read brand from URL if client navigation occurred.
   */
  useEffect(() => {
    try {
  ...
  /*
   * Load wishlist asynchronously without blocking initial page rendering.
   */
  useEffect(() => {
    fetch("/api/wishlist", {
      cache: "no-store",
  ...
  const toggleWishlist = async (event, product) => {
    ...
    if (!product?.id || wishlistLoading[product.id]) {
      return;
    }
                  ...
                  <div className="relative aspect-[4/5] overflow-hidden rounded-2xl bg-neutral-950">
                    ...
                    <div className="pointer-events-none absolute left-3 top-3 z-10 flex flex-col gap-1">
                      {(() => {
                        ...
                        return (
                          <span
                            className={`rounded-full px-3 py-1 text-[9px] font-black uppercase tracking-wider backdrop-blur border ${stockStatus.colorClass}`}
                          >
                            {stockStatus.label}
                          </span>
                        );
                    ...
                    <div className="absolute right-3 top-3 z-20 flex flex-col gap-2">
                      ...
                      {!isOutOfStock && (
                        <button
                          type="button"
                          onClick={(event) =>
                            handleProductCardAction(event, product)
                          }
                          disabled={isCartBusy}
                          aria-label={
