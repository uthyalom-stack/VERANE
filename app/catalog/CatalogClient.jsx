...
export default function CatalogClient({
  ...
  /*
   * Read brand from URL if specified on client navigation.
   */
  useEffect(() => {
    try {
      ...
      console.error("Failed to read catalog URL:", error);
    }
  }, []);

  /*
   * Load wishlist in background without blocking rendering.
   */
  useEffect(() => {
    fetch("/api/wishlist", {
      cache: "no-store",
      ...
      .then((wishlistData) => {
        if (!wishlistData || !Array.isArray(wishlistData.wishlist)) return;
  ...
  const filtered = useMemo(() => {
    ...
    const result = products.filter((product) => {
      ...
      return matchBrand && matchCat && matchCollection && matchSearch;
    });
  ...
  const toggleWishlist = async (event, product) => {
    ...
    if (!product?.id || wishlistLoading[product.id]) return;

    const productId = product.id;
    const oldState = Boolean(wishlist[productId]);
    const newState = !oldState;

    setWishlist((previous) => ({ ...previous, [productId]: newState }));
    setWishlistLoading((previous) => ({ ...previous, [productId]: true }));
    ...
    try {
      ...
      if (response.status === 401) {
        setWishlist((previous) => ({ ...previous, [productId]: oldState }));
      ...
      setWishlist((previous) => ({
        ...previous,
        [productId]: Boolean(data.wishlisted),
      }));
    } catch (error) {
      console.error("Wishlist update failed:", error);
      setWishlist((previous) => ({ ...previous, [productId]: oldState }));
    } finally {
      setWishlistLoading((previous) => ({ ...previous, [productId]: false }));
    }
  };
   ...
   */
  const handleProductCardAction = (event, product) => {
    event.preventDefault();
    event.stopPropagation();
    ...
    setCartLoading((previous) => ({ ...previous, [product.id]: true }));
    ...
    try {
      ...
      setTimeout(() => {
        setCartLoading((previous) => ({ ...previous, [product.id]: false }));
      }, 350);
    }
  };
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-10 lg:grid-cols-4">
            {filtered.map((product) => {
              ...
              const isWishlistBusy = Boolean(wishlistLoading[product.id]);
              const isCartBusy = Boolean(cartLoading[product.id]);
                  ...
                  <div className="relative aspect-[4/5] overflow-hidden rounded-2xl bg-neutral-950">
                    ...
                    <div className="absolute right-3 top-3 z-20 flex flex-col gap-2">
                      ...
                      <button
                        ...
                        } ${isWishlistBusy ? "cursor-wait opacity-70" : ""}`}
                      >
                        {isWishlistBusy ? (
                          <Spinner />
                        ) : (
                      ...
                      {!isOutOfStock && (
                        <button
                          type="button"
                          onClick={(event) => handleProductCardAction(event, product)}
                          disabled={isCartBusy}
                          ...
                          className={`flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-black/60 text-white backdrop-blur-xl transition-all hover:border-amber-400/70 hover:bg-amber-500 hover:text-black ${
                            isCartBusy ? "cursor-wait opacity-70" : ""
                          }`}
                        >
                          {isCartBusy ? <Spinner /> : <CartIcon />}
                        </button>
                      )}
                    </div>
