...
/**
 * Direct order creation is disabled for financial security.
 * All customer orders must be initialized via POST /api/paystack/initialize.
 * @returns {NextResponse} An HTTP 403 response explaining the requirement.
 */
  ...
  return NextResponse.json(
    {
      success: false,
      error:
        "Direct order creation is disabled. All orders must be initialized via the authorized Paystack checkout pipeline (/api/paystack/initialize).",
    },
    { status: 403 }
  );
}