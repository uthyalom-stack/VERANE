...
export async function PUT(request, { params }) {
  try {
    ...
    const existing = await prisma.category.findFirst({ where: { id, brand: admin.brand } });
    ...
    const sizeType = typeof body.sizeType === "string" ? body.sizeType.trim().toLowerCase() : existing.sizeType || "none";

    if (!name || !slug) return NextResponse.json({ success: false, error: "A valid category name is required." }, { status: 400 });
    if (!VALID_SIZE_TYPES.has(sizeType)) return NextResponse.json({ success: false, error: "Please choose a valid sizing type." }, { status: 400 });

    const duplicate = await prisma.category.findFirst({ where: { brand: admin.brand, slug, NOT: { id } } });
    if (duplicate) return NextResponse.json({ success: false, error: "Another category with this name already exists." }, { status: 409 });

    const category = await prisma.category.update({ where: { id }, data: { name, slug, description: description || null, enabled, sortOrder, sizeType } });
